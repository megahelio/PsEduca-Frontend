    ...
    const intranet_section = `
        <li class="list-item-with-children" id="list-item-with-children">
            ...
            <ul class="sublist" id="sublist">
                ...
                ${ROL === 'ADMIN_GLOBAL' ? '<li><a href="javascript:void(0);" data-i18n="header.navbar.divulgationManagement">Gestión divulgación</a></li>' : ''}
...
async function loadDivulgations(){
    const divulgations = await getDivulgations();
    const $divulgationsTable = $('#divulgation-management-table > tbody');

    if(!divulgations.error){
        divulgations.forEach(divulgation => {
            $divulgationsTable.innerHTML += `
                <tr data-divulgation-id="${divulgation.id}">
                    <td>${divulgation.title}</td>
                    <td style="text-overflow: ellipsis;">${divulgation.description?.substring(0, 50) || ''}...</td>
                    <td style="text-overflow: ellipsis;">${divulgation.content?.replace(/<[^>]*>?/gm, '')?.substring(0, 50) || ''}...</td>
                    <td>
                        ${divulgation.link ? `<a href="${divulgation.link}" target="_blank">Divulgación</a>` : 'Sin enlace'}
                    </td>
                    <td style="text-align:center;">
                        <a href="${divulgation.image}" target="_blank">
                            <img src="${divulgation.image}" width="50px"/>
                        </a>
                    </td>
                    <td class="actions-cell-table">
                        <a class="access-divulgation-link" href="./detail/index.html?id=${divulgation.id}">
