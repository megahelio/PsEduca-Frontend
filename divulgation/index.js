...
async function loadDivulgation(){
    const response = await getDivulgations();
    ...
    const $divulgationList = $('#divulgation-list');

    response.forEach(divulgation => {
        const linkResource = divulgation.link || `./detail/index.html?id=${divulgation.id}`;

        $divulgationList.innerHTML += `
            <li class="divulgation-list-item">
                <a href="${linkResource}" ${divulgation.link ? 'target="_blank"': ''} class="divulgation-card">
                    <div class="divulgation-image-container">
                        <img src="${divulgation.image}" class="divulgation-image" alt="${divulgation.title}"/>
                    </div>
                    <div class="divulgation-text-container">
                        <h2>${divulgation.title}</h2>
                        <p>${divulgation.description}</p>
                    </div>
                </a>
            </li>
        `;
